...
void main() {
	...
	vec3 shadow;
	float fragDist = length(feetPlayerPos);

	if (fragDist > 48.0) {
		shadow = vec3(step(shadowScreenPos.z, texture(shadowtex0, shadowScreenPos.xy).r));
	} else {
	...
	vec3 nearbySourceColor = vec3(0.0);
	if (lightmap.x > 0.1) {
		nearbySourceColor = findNearbySourceColor(texcoord);
	}
